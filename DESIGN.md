...
- DB は **Postgres** を使う(Supabase Postgresを含む)
...
- **カレンダーUIで候補日選択** — 最大のUX改善点。カレンダーをタップして複数選択し、各日に開始時刻を付与。時刻はプリセット(19:00〜 など)のクイック追加でも、`<input type="time">` の自由入力・編集でも設定でき、「終日」も選べる(テキスト/カレンダーのモード分けは廃止し本方式に一本化)
- **自動更新集計** — 誰かが回答すると開いている画面が約5秒間隔で更新される
- **モバイルファースト** — 回答フォームは縦積みカード、集計表は先頭列固定の横スクロール
- **確定 → カレンダー連携** — 幹事が日程を確定すると .ics ダウンロードと Google カレンダー追加リンクを表示
- **OGP画像自動生成**(next/og) — 共有先で回答件数入りの固定カードが出る（UGCは外部フォント取得へ渡さない）
- ダークモード対応
...
| DB | Postgres(Supabase Postgresを含む) | マネージドPostgresまたはローカルPostgresを利用できる |
| ORM | Drizzle | 型安全・軽量 |
...
| `/e/[slug]` | イベントページ。RSCで集計表を描画し、client component が約5秒間隔でポーリング |
...
集計ページは約5秒間隔のポーリングで `router.refresh()` を実行して RSC を再取得する
(クライアント側で差分マージせず、サーバーを真実の源にする)。非表示タブでは
ポーリングを停止する。
...
2. **Phase 2 (体験差別化)**: カレンダー選択UI、自動更新、モバイル回答カード、ダークモード
